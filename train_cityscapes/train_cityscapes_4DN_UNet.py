...
train_images_names_original = sorted([img for img in os.listdir(train_images_folder_path) if img.endswith('_0.02.png')])
train_mask_names_original = sorted([img for img in os.listdir(train_mask_folder_path) if img.endswith('.png')])
test_images_names = sorted([img for img in os.listdir(test_images_folder_path) if img.endswith('_0.02.png')])
...
def load_image(folder, file, height = 96, width = 256, crop = True):
    ...
    # Crop the image
    if crop:
        image = tf.image.crop_to_bounding_box(image, 0, 512, 512, 1024)   #image, 0, 0, 768, 2048
...
# Starts training models...

seeds = [0, 70, 8, 123]    # 0, 11, 25, 333, 41, 55, 666, 70, 8, 123
...
for i in range(len(seeds)):
    ...
    class JaccardScoreCallback(keras.callbacks.Callback):
        ...
        def on_epoch_end(self, batch, logs=None):
            ...
            if jaccard_value > self.best_iou:
              ...
              self.model.save_weights('./New_4_DN_segmentation/Train_' + str(i) + '/best_model.h5', overwrite = True)
    ...
    def visualize_image_mask(image_paths, mask_paths, preds, colors, img_height = 96, img_width = 256):
        ...
        plt.savefig('./New_4_DN_segmentation/Train_' + str(i) + '/Predictions.png')
        #plt.show()
        plt.clf()
    ...
    np.save('./New_4_DN_segmentation/Train_' + str(i) + '/history_train_' + str(i) + '.npy', history_model1.history)
    np.save('./New_4_DN_segmentation/Train_' + str(i) + '/ious_train_' + str(i) + '.npy', np.asarray(ious))
    ...
    plt.ylabel('Log10(lr)')
    plt.xlabel('Epoch')
    plt.savefig('./New_4_DN_segmentation/Train_' + str(i) + '/Lr.png')
    #plt.show()
    plt.clf()
    ...
    plt.ylabel('Accuracy')
    plt.xlabel('Epoch')
    plt.savefig('./New_4_DN_segmentation/Train_' + str(i) + '/Accuracy.png')
    #plt.show()
    plt.clf()
    ...
    plt.ylabel('Loss')
    plt.xlabel('Epoch')
    plt.savefig('./New_4_DN_segmentation/Train_' + str(i) + '/Loss.png')
    #plt.show()
    plt.clf()
    ...
    plt.ylabel('IoU')
    plt.xlabel('Epoch')
    plt.savefig('./New_4_DN_segmentation/Train_' + str(i) + '/Iou_validation.png')
    #plt.show()
    plt.clf()

    # Restore best weights
    model1.load_weights('./New_4_DN_segmentation/Train_' + str(i) + '/best_model.h5')
    ...
    # Mean IoU
    #print('Mean IOU: ', iou_metrics(y_real, preds_buenas))
    #print('Max IoU validation: ' + str(np.max(ious)))
